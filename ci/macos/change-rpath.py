    ...
    def copy_dependencies(self, f, src=None):
        ...
        for lib in libs:
            ...
            if not os.path.exists(dest):
                ...
                if self.args.license_dir:
                    lic_dest_prefix = self.args.license_dir + '/' + base + '-'
                    re_licfile = re.compile(r'(COPYING|LICENSE)(\.md|\.txt)?$')
                    cand_dir = os.path.dirname(lib) + '/..'
                    for licfile in os.listdir(cand_dir):
                        if re_licfile.match(licfile):
                            os.makedirs(self.args.license_dir, exist_ok=True)
                            shutil.copy(cand_dir + '/' + licfile, lic_dest_prefix + licfile)
...
def _get_args():
    ...
    parser.add_argument('--license-dir', action='store', default=None)
